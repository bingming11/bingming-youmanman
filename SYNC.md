...
| 源 | 内容 | 乐享里的位置 |
| --- | --- | --- |
| **主 Excel 附件** | `邮满满云仓+云途特惠+欧美专线0810(3).xls`（含「云仓报价」工作表 + 18 张云途线路表 + 云仓售后） | 挂在「邮满满云仓报价」宿主页（`864f83b2391840709e336a57904b825a`）上，file_id `f01057724a4a492c8e313131a6030efa` |
...
> ⚠️ 暂不支持：单独在乐享**上传一个全新的 `.xls/.xlsx` 文件**。乐享 MCP 不提供"列出文件附件"接口，无法自动发现它。
> 若你确实需要上传独立的新 Excel 文件，告诉我一声，我把它的 file_id 加进拉取清单即可（一次性）。
